...
The methods used are similar, but the `bboushiny` GUI is more user-friendly and `bboutools`, `bbouretro` and `bbousims` R packages offer more fine-grained control of analyses and the ability to compare methods. 
...
# Overview of methods
## Survival
Survival is estimated from the monthly fate of collared adult females, with an option to include uncertain mortalites in the total monthly mortalities summed prior to model fitting. 
